...
static RedGlzDrawable *red_display_get_glz_drawable(DisplayChannel *channel, Drawable *drawable)
{
    ...
    surface = &channel->common.worker->surfaces[drawable->surface_id];
...
static void display_channel_hold_item(PipeItem *item)
{
    ASSERT(item);
    switch (item->type) {
    case PIPE_ITEM_TYPE_DRAW:
...
static void cursor_channel_hold_item(PipeItem *item)
{
    ASSERT(item);
